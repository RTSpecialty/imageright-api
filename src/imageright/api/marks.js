...
export function getFileMarkDefinitionsByFileTypeIdV2(api, fileTypeId) {
  return api
    .get(`api/v2/objectypes/${fileTypeId}/filemarks`)
...
export function getPageMarkDefinitions(api) {
  return api.get('api/v2/pagemarks').then((res) => Promise.resolve(res.data));
}

export function getPageMarkDefinitionsByFileTypeId(api, fileTypeId) {
  return api
    .get(`api/v2/objectypes/${fileTypeId}/pagemarks`)
    .then((res) => Promise.resolve(res.data));
}

export function getPageMarksForFile(api, fileId) {
  return api.get(`api/v2/files/${fileId}/pagemarks`).then((res) => Promise.resolve(res.data));
}
